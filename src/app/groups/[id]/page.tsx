...
import {RatingPlanPanel} from "@/components/RatingPlanPanel";
...
export default async function GroupDetailPage({params}: GroupDetailPageProps) {
    ...
    const plans: RatingPlanEntry[] = isMember
        ? await getGroupRatingPlans(supabase, id)
        : [];
        ...
        <div className="max-w-4xl mx-auto px-4 py-8 w-full">
                ...
                <div className="bg-night-800/60 border border-night-700 rounded-xl p-4">
                    ...
                    {!group.spotify_client_id ? (
                        <p className="text-xs text-zinc-500">
                            ...
                            configured. {isOwner && "Add them using the form below so all members can search and play."}
                        </p>
                    ) : (
                    ...
                    {isOwner && group &&
                        <GroupCredentialsForm groupId={group.id} initialClientId={group.spotify_client_id}/>}
                </div>
            </div>
                ...
                <div className="space-y-2">
                    {memberProfiles.map((profile) => (
                        ...
                        >
                            ...
                            {profile.id === userId && (
                                <span
                                    className="text-[10px] bg-night-700 text-zinc-400 px-1.5 py-0.5 rounded-full ml-auto">
                  you
                </span>
                            )}
            ...
            <div className="mt-6">
                <RatingPlanPanel
                    groupId={id}
                    plans={plans}
                    members={memberProfiles.map((m) => ({id: m.id, username: m.username}))}
                />
            </div>
        </div>
    );
