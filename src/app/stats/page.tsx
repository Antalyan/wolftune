...
export default async function StatsPage({searchParams}: StatsPageProps) {
    ...
    let plans: RatingPlanEntry[] = [];
    if (selectedGroupId) {
        [bestTracks, bestAlbums, bestPlaylists, similarities, plans] = await Promise.all([
            ...
            getGroupPlansForStats(supabase, selectedGroupId),
        ]);
    }
                ...
                <>
                    ...
                    {selectedGroupName && (
                        <p className="text-xs text-zinc-500 mb-4">
                            Showing statistics for <span
                            className="text-zinc-300 font-semibold">{selectedGroupName}</span>.
                        </p>
                    )}
...
function StatsRow({rank, item}: { rank: number; item: BestItem }) {
    ...
    const content = (
        <div
            className="p-3 rounded-xl bg-night-800/80 border border-night-700 hover:border-night-600 transition-colors">
        ...
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-sm text-white truncate">{item.title}</span>
            ...
            <div className="h-1.5 rounded-full bg-night-900 overflow-hidden mb-1.5">
                <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-spotify-green"
...
function PlansSection({plans}: { plans: RatingPlanEntry[] }) {
    const fmt = (iso: string) => {
        const d = new Date(iso + "T00:00:00");
        if (Number.isNaN(d.getTime())) return iso;
        return d.toLocaleDateString(undefined, {weekday: "short", month: "short", day: "numeric"});
    };
    const upcoming = plans.filter((p) => !p.done);
    ...
    return (
        <section>
            <h2 className="flex items-center gap-2 text-sm font-bold text-zinc-300 mb-3">
                ...
                Planned ratings ({plans.length})
            </h2>
            {plans.length === 0 ? (
                <p className="text-xs text-zinc-500">
                    No plans yet. Add entries on the group page to schedule what to rate next.
                </p>
            ) : (
                <div className="space-y-2">
                    {upcoming.length > 0 && (
                        <>
                            ...
                            {upcoming.map((p) => (
                                <PlanRow key={p.id} plan={p} fmt={fmt}/>
                            ))}
                        </>
                    )}
                    {completed.length > 0 && (
                        <>
                            <p className="text-[10px] uppercase tracking-wider text-zinc-500 mt-3 mb-1">
...
function PlanRow({plan, fmt}: { plan: RatingPlanEntry; fmt: (iso: string) => string }) {
    const TargetIcon = plan.target.kind === "album" ? Disc3 : ListMusic;
    return (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-night-800/60 border border-night-700">
            <div
                className="w-9 h-9 rounded-lg bg-night-900 border border-night-700 overflow-hidden shrink-0 flex items-center justify-center">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                    ...
                    {plan.done && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0"/>}
                </div>
                <div className="flex items-center gap-2 text-[11px] text-zinc-400">
                    <span className="truncate">{plan.target.subtitle}</span>
                    <span className="text-zinc-600">·</span>
                    <span>{plan.assignedMember.username}</span>
                    <span className="text-zinc-600">·</span>
                    <span>{fmt(plan.scheduledDate)}</span>
                </div>
            </div>
            {!plan.done && <Clock className="w-4 h-4 text-zinc-500 shrink-0"/>}
        </div>
    );
}
